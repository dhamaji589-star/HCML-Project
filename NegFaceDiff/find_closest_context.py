...
class CenterGeneratorLite(LightningLite):
    def run(self, cfg) -> Any:
        ...
        # determining the closest imposter context for each context
        for prefix in range(0, 2):
            ...
            cos_sim[prefix] = -1
            closest_context_idx = torch.argmax(cos_sim)

            dict_closest_cossim[prefix] = closest_context_idx

        np.save(os.path.join(save_dir, "closest_cossim.npy"), dict_closest_cossim)

...
parser.add_argument('--network', default='iresenet50', type=str, help='')
...
from backbones.iresnet import iresnet50, iresnet34
...
class Embedding(object):
    def __init__(self, prefix, data_shape, batch_size=1):
        ...
        # load model
        if network == "mfn":
            backbone = mobilefacenet.MobileFaceNet(input_size=[112, 112], embedding_size=512).to(f"cuda:{gpu_id}")
        elif network == "iresenet50":
            backbone= iresnet50(num_features=512, use_se=False).to(f"cuda:{gpu_id}")
        elif network == "iresnet34":
            backbone= iresnet34(num_features=512, use_se=False).to(f"cuda:{gpu_id}")
        else:
            backbone = None
            exit()
        ...
        weight = torch.load(prefix, map_location=torch.device(f"cuda:{gpu_id}"))

...
class CenterGeneratorLite(LightningLite):
    def run(self, cfg) -> Any:
        ...
        # determining the imposter context that as a median position for each context
        for prefix in range(0, contexts_matrix.shape[0]):
            ...
            sorted_indices = torch.argsort(cos_sim)  # sorts from lowest to highest
            middle_index = sorted_indices[len(sorted_indices) // 2]  # get the middle element
            
            dict_middle_cossim[prefix] = middle_index

        np.save(os.path.join(save_dir, "median_cossim.npy"), dict_middle_cossim)
